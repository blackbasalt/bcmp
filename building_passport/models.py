...
class FloorPlan(CommonModel):
    ...
    @property
    def period(self):
        """Период плана: «01.01.2020 — 31.12.2026» или «01.01.2020 — по сей день».

        Одно описание периода на весь план: им отказывают пересекающемуся плану, им
        же экран этажа объясняет, почему выбранный день лежит вне чертежа. Второе
        описание того же периода однажды разошлось бы с первым — в отказе стояло бы
        одно, на экране другое. Пустой конец означает «действует», а не «не
        действовал никогда» (ADR 0004).

        Про договор аренды то же самое говорит `lease_display.period`, и общего кода
        у них нет намеренно — по той же причине, по какой его нет у двух `overlapping`:
        периоды у плана и у договора совпадают формой, а не природой, и склейка ради
        двух строк связала бы паспорт с арендой на ровном месте.
        """
        closes = f"{self.valid_to:%d.%m.%Y}" if self.valid_to else "по сей день"
        return f"{self.valid_from:%d.%m.%Y} — {closes}"

    def in_force_on(self, day):
        """Действует ли этот план в названный день — тот же вопрос, что и у выборки.

        Спрашивают его порознь: выборка отвечает «какой план действует» на весь этаж,
        а сам план — «а этот?», когда он уже в руках у экрана. Правило периода при
        этом остаётся одно и лежит на плане, а не переписывается на стороне
        спрашивающего: оба конца входят в период, пустой конец означает «по сей день»
        (ADR 0004).
        """
    ...
    def _validate_period(self):
        ...
        if conflicting is not None:
            raise ValidationError(
                f"Период пересекается с планом этажа за {conflicting.period}. У этажа "
                f"не бывает двух действующих планов: закройте прежний период датой "
                f"перепланировки."
            )
