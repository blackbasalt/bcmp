"""Список БЦ — то, что сотрудник УК видит по HTTP.

Шов один: граница HTTP. Тесты ходят тестовым клиентом по именованным адресам от
имени пользователя с известным членством и проверяют наблюдаемое — какие
бизнес-центры на экране и какой код ответа. Разметка, классы и заголовки не
проверяются, чтобы перестройка ниже уровня URL не переписывала набор тестов.
"""
...
def test_a_member_sees_their_own_portfolio_and_not_another_clients(client, member, downtown, central):
    """Изоляция клиентов на экране — ровно то, ради чего вводился чокпоинт."""
...
def test_a_user_without_membership_gets_an_empty_list_and_an_explanation(
    ...
    """Новому сотруднику нужен не 403 и не чужие данные, а объяснение, к кому идти."""
    make_building(downtown, "Manhattan")
...
def test_the_list_holds_business_centres_only(client, member, downtown):
    """Навигация плоская: площадка в списке не появляется."""
    make_building(downtown, "Manhattan")
...
def test_a_card_shows_what_tells_the_buildings_apart(client, member, downtown):
    """Наименование, адрес, класс, год постройки и общая площадь — прямо на карточке."""
    ...
    assert "B+" in page
    assert "2017" in page
    assert "2\u00a0484,10\u00a0м²" in page  # как в паспорте, неразрывными пробелами


def test_a_missing_passport_value_reads_as_no_data_rather_than_as_a_blank(
    client, member, downtown
):
    """Пустое место на карточке можно прочитать как ноль; «— нет данных» — нельзя."""
    ...
    assert page.count("— нет данных") == 2  # год постройки и общая площадь


def test_a_building_whose_spaces_are_not_loaded_carries_a_badge(client, member, downtown):
    """Четыре БЦ из пяти пока без помещений — это нормальное состояние, не ошибка."""
...
def test_a_card_opens_the_passport_of_its_building(client, member, downtown):
    """Открыть БЦ — щелчок по карточке, а не поиск в меню."""
    building = make_building(downtown, "Manhattan")
    client.force_login(member)
...
def test_the_project_is_a_label_and_the_site_does_not_appear(client, member, downtown):
    """Навигация плоская: проект — подпись, площадка не показывается вовсе."""
...
def test_another_clients_row_does_not_clear_the_badge(client, member, downtown, central):
    """Значок снимает только то, что видно самому пользователю (ADR 0001)."""
    building = make_building(downtown, "Manhattan")
    Space.objects.create(
