...
class LeaseSubjectQuerySet(models.QuerySet):
    ...
    def in_force_on(self, day):
        """Предметы, чьи договоры действуют в названный день, — отрезок в один день.

        Спрашивают это трижды — слоем сроков, счётом свободного и карточкой
        помещения, — и спрашивают одно и то же: «сдано ли это помещение в этот
        день». Три написания одного вопроса разошлись бы на первом же уточнении
        правила, поэтому написание одно и лежит рядом с самим правилом периода
        (ADR 0007). Тем же именем и по той же причине отвечает `FloorPlanQuerySet`
        про действующий план этажа.

        По одному помещению таких предметов не больше одного: периоды договоров на
        одно помещение не пересекаются. По этажу их столько, сколько сдано.
        """
