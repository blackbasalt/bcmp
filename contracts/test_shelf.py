...
#: организации, его право заводить договоры, сами строки — и по паре на каждый из двух
#: списков полосы отбора, контрагентов и БЦ, которые `{% for %}` сперва считает, а потом
#: перечисляет. Счёт всей полки своего запроса не стоит: несуженная полка — тот же набор, что
#: уже вычитан строками, и `count` читает его кэш. Право спрашивается и у читателя без флага:
#: показать створку или нет — вопрос о нём, а не о том, что на полке. Числом, а не потолком:
#: потолок с запасом пропустил бы запрос на договор ровно до того дня, когда договоров станет
#: достаточно, чтобы это стало видно на экране.
QUERIES_PER_SHELF = 9
...
def test_the_shelf_costs_no_query_per_row(
    ...
    были бы двумя запросами на договор при сотнях договоров. Списки полосы отбора стоят
    своего, но их цена — от числа контрагентов и БЦ, а не от числа строк."""
...
def test_the_rows_of_the_shelf_carry_no_way_to_change_anything(shelf_page, our_contracts):
    """Строка отвечает, а не предлагает. Договор заводят одной створкой над таблицей, а правят
    и удаляют на экране самого договора: рейка правки в каждой строке превратила бы полку,
    которую читают глазами, в бланк на сотню записей."""
    filled, _ = our_contracts
