...
#: организации, сами строки — и по паре на каждый из двух списков полосы отбора, контрагентов
#: и БЦ, которые `{% for %}` сперва считает, а потом перечисляет. Счёт всей полки своего
#: запроса не стоит: несуженная полка — тот же набор, что уже вычитан строками, и `count`
#: читает его кэш. Числом, а не потолком: потолок с запасом пропустил бы запрос на договор
#: ровно до того дня, когда договоров станет достаточно, чтобы это стало видно на экране.
QUERIES_PER_SHELF = 8
